        ...
        <div className="max-w-sm w-full bg-white rounded-xl shadow border border-red-200 overflow-hidden">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-500 text-sm">
              This document could not be verified. It may be forged, modified,
              or not issued by PayrollPro.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-xl shadow overflow-hidden">
            <div className="px-5 py-4 border-b">
              <h2 className="font-semibold text-gray-900 flex items-center gap-2">
                ...
                All Salary Slips — {payslip.employee?.firstName}{" "}
                {payslip.employee?.lastName}
              </h2>
              <p className="text-xs text-gray-500 mt-0.5">
                Tap any slip to view it
              </p>
            </div>
                  ...
                  {h.verificationToken ? (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-2 h-2 rounded-full flex-shrink-0 ${h.isCurrent ? "bg-blue-500" : "bg-gray-300"}`}
                        />
                        <div>
                          <div className="text-sm font-medium text-gray-900">
                            {h.referenceNumber || fmtMonth(h.month, h.year)}
                            {h.isCurrent && (
                              <span className="ml-2 text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-normal">
                                current
                              </span>
                            )}
                          </div>
                          <div className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                            ...
                            {h.issueDate
                              ? fmtDate(h.issueDate)
                              : fmtMonth(h.month, h.year)}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-gray-900">
                          {fmtAud(h.totalAmount)}
                        </span>
