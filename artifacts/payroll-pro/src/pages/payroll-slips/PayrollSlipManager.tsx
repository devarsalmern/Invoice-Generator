...
import { FilePlus2, Printer, Save, Trash2, WalletCards } from "lucide-react";
...
const storageKey = "payrollpro_employee_payroll_slips";
...
const blank = (): PayrollSlip => ({
  id: crypto.randomUUID(), companyId: "", employeeId: "", companyName: "", companyAbn: "", employeeName: "",
  employeeNumber: "", employeeAddress: "", periodStart: date(), periodEnd: date(), datePaid: date(), payRate: "0",
  hours: "0", earningsName: "Permanent Ordinary Hours", earningsNote: "", ytdEarnings: "0", payg: "0", ytdPayg: "0",
  superFund: "AustralianSuper", superType: "Super Guarantee", superMemberNumber: "", superAmount: "0", ytdSuper: "0",
  ...
  return (
    <article className="payroll-slip bg-white text-slate-900 p-8 md:p-10 text-sm min-h-[297mm]">
      ...
      <Section title="Pay Slip Components" headers={["Hours/Units", "Rate", "This Pay", "Year To Date"]}>
        ...
        <div className={`${row} py-2`}><span>PAYG</span><span /><span /><span className="text-right">{money(slip.payg)}</span><span className="text-right">{money(slip.ytdPayg)}</span></div>
        <Total values={[money(slip.payg), money(slip.ytdPayg)]} />
        <h3 className="font-bold text-base mt-5">Superannuation Breakdown</h3>
        <div className={`${row} py-2`}><span>SG</span><span /><span /><span className="text-right">{money(slip.superAmount)}</span><span className="text-right">{money(slip.ytdSuper)}</span></div>
      ...
      <div className="mt-auto pt-64 text-right text-xs text-slate-600">Employee Id: <b>{slip.employeeNumber || "—"}</b></div>
    </article>
  );
}
...
export default function PayrollSlipManager() {
  ...
  const [slips, setSlips] = useState<PayrollSlip[]>(() => { try { return JSON.parse(localStorage.getItem(storageKey) || "[]"); } catch { return []; } });
  ...
  useEffect(() => localStorage.setItem(storageKey, JSON.stringify(slips)), [slips]);
  ...
  const save = () => { if (!editing.companyId || !editing.employeeId) { toast({ variant: "destructive", title: "Select a company and employee first" }); return; } setSlips((all) => [editing, ...all.filter((x) => x.id !== editing.id)]); toast({ title: "Employee payslip saved" }); };
  ...
  return <div className="space-y-6 max-w-[1500px] mx-auto">
    ...
    <div className="grid xl:grid-cols-[440px_minmax(0,1fr)] gap-6 items-start"><Card><CardHeader><CardTitle>Pay slip details</CardTitle><CardDescription>Every field is reflected in the print-ready payslip.</CardDescription></CardHeader><CardContent className="space-y-6"><FieldGroup title="Employee & period"><div className="grid gap-3"><Select value={editing.companyId} onValueChange={selectCompany}><SelectTrigger><SelectValue placeholder="Select company" /></SelectTrigger><SelectContent>{companies?.map((c) => <SelectItem value={String(c.id)} key={c.id}>{c.name}</SelectItem>)}</SelectContent></Select><Select value={editing.employeeId} onValueChange={selectEmployee} disabled={!editing.companyId}><SelectTrigger><SelectValue placeholder="Select employee" /></SelectTrigger><SelectContent>{employees?.map((e) => <SelectItem value={String(e.id)} key={e.id}>{e.firstName} {e.lastName}</SelectItem>)}</SelectContent></Select><DateFields slip={editing} set={set} /></div></FieldGroup><FieldGroup title="Earnings"><div className="grid grid-cols-2 gap-3"><Text label="Hourly rate" value={editing.payRate} set={(v) => set("payRate", v)} type="number" /><Text label="Hours paid" value={editing.hours} set={(v) => set("hours", v)} type="number" /><Text label="Earnings label" value={editing.earningsName} set={(v) => set("earningsName", v)} /><Text label="YTD earnings" value={editing.ytdEarnings} set={(v) => set("ytdEarnings", v)} type="number" /></div><p className="text-sm font-medium pt-2">This pay: {money(gross)}</p><Text label="Notes" value={editing.earningsNote} set={(v) => set("earningsNote", v)} /></FieldGroup><FieldGroup title="Tax & super"><div className="grid grid-cols-2 gap-3"><Text label="PAYG this pay" value={editing.payg} set={(v) => set("payg", v)} type="number" /><Text label="YTD PAYG" value={editing.ytdPayg} set={(v) => set("ytdPayg", v)} type="number" /><Text label="Super fund" value={editing.superFund} set={(v) => set("superFund", v)} /><Text label="Member number" value={editing.superMemberNumber} set={(v) => set("superMemberNumber", v)} /><Text label="Super this pay" value={editing.superAmount} set={(v) => set("superAmount", v)} type="number" /><Text label="YTD super" value={editing.ytdSuper} set={(v) => set("ytdSuper", v)} type="number" /></div><Text label="Contribution type" value={editing.superType} set={(v) => set("superType", v)} /></FieldGroup><FieldGroup title="Bank payment"><div className="grid grid-cols-2 gap-3"><Text label="Payment method" value={editing.paymentMethod} set={(v) => set("paymentMethod", v)} /><Text label="Account number" value={editing.bankAccount} set={(v) => set("bankAccount", v)} /></div></FieldGroup></CardContent></Card>
      <div className="space-y-4"><div className="flex justify-between items-center"><div className="font-semibold">Live print preview</div><Button variant="outline" onClick={print}><Printer className="w-4 h-4 mr-2" />Print / Save PDF</Button></div><Card className="overflow-hidden shadow-md"><SlipPreview slip={editing} /></Card></div></div>
    {slips.length > 0 && <Card><CardHeader><CardTitle className="flex items-center gap-2"><WalletCards className="h-5 w-5" />Saved employee pay slips</CardTitle></CardHeader><CardContent className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">{slips.map((slip) => <button className="text-left border rounded-lg p-4 hover:bg-muted/50" key={slip.id} onClick={() => setEditing(slip)}><div className="flex justify-between gap-2"><strong>{slip.employeeName}</strong><Badge variant="secondary">{formatDate(slip.datePaid)}</Badge></div><p className="text-sm text-muted-foreground mt-1">{slip.companyName} · {money(number(slip.payRate) * number(slip.hours))}</p><span onClick={(e) => { e.stopPropagation(); setSlips((all) => all.filter((x) => x.id !== slip.id)); }} className="inline-flex mt-3 text-xs text-destructive items-center gap-1"><Trash2 className="w-3 h-3" />Delete</span></button>)}</CardContent></Card>}
  </div>;
}
