...
export const payslipsTable = pgTable("payslips", {
  ...
  // Payroll statement snapshot fields. These are stored with the payslip so a
  // historical statement does not change when an employee profile is updated.
  ...
  earningsName: text("earnings_name"),
  earningsNote: text("earnings_note"),
  ytdEarnings: numeric("ytd_earnings", { precision: 12, scale: 2 }),
  payg: numeric("payg", { precision: 12, scale: 2 }),
  ytdPayg: numeric("ytd_payg", { precision: 12, scale: 2 }),
  superFund: text("super_fund"),
  superName: text("super_name"),
  superType: text("super_type"),
  superMemberNumber: text("super_member_number"),
  superAmount: numeric("super_amount", { precision: 12, scale: 2 }),
  ytdSuper: numeric("ytd_super", { precision: 12, scale: 2 }),
...
export const payslipItemsTable = pgTable("payslip_items", {
  ...
  description: text("description")
    .notNull()
    .default("Daily subcontract painting services"),
